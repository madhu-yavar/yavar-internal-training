...
import { Code, BookOpen, TrendingUp, CheckCircle2, Clock, Filter } from 'lucide-react';
...
function PracticeHub() {
  ...
  const [filteredExercises, setFilteredExercises] = useState<SandboxExercise[]>([]);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100">
        ...
        <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-4 sm:px-6">
          <Link to="/" className="flex items-center gap-2">
            ...
            <div className="border-l border-white/10 pl-2">
              <div className="text-sm font-semibold leading-none">Python Sandbox</div>
              <div className="text-[10px] uppercase tracking-[0.2em] text-amber-300">Practice Coding</div>
            </div>
          </Link>
          <div className="flex items-center gap-2 text-sm">
            ...
            <Link
              to="/learn"
              className="px-3 py-1.5 rounded-md border border-slate-700 hover:bg-slate-800 text-xs"
            >
              Courses
            </Link>
            {isAdmin && (
              <Link to="/admin/sandbox" className="rounded-md border border-amber-400/40 bg-amber-500/10 px-3 py-1.5 text-xs text-amber-300 hover:bg-amber-500/20">
                Manage
              </Link>
            )}
          </div>
      ...
      <main className="mx-auto max-w-6xl px-4 py-8 sm:px-6">
        {/* Hero Section */}
        <div className="mb-8 rounded-2xl border border-amber-400/30 bg-gradient-to-br from-amber-500/10 to-rose-500/10 p-6 sm:p-8">
          <div className="flex items-start justify-between gap-4">
            <div className="flex-1">
              <h1 className="text-2xl font-bold text-amber-200 sm:text-3xl">Python Coding Sandbox</h1>
              <p className="mt-2 text-slate-300">
                Practice Python right in your browser! Write code, run tests, and get AI-powered feedback.
          ...
          <div className="mt-6 grid grid-cols-3 gap-4 sm:gap-8">
            <div className="text-center">
              <div className="text-2xl font-bold text-amber-300">{totalCount}</div>
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <h3 className="text-lg font-semibold group-hover:text-amber-300 truncate">
                          {exercise.title}
                        </h3>
                        {isCompleted && (
                    ...
                    <div className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-xl border border-slate-700 bg-slate-800">
